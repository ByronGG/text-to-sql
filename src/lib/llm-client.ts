...
import type { Lang } from "@/lib/llm-schema";
...
export async function fetchSuggestions(tables: TableSchema[], lang: Lang = "es"): Promise<string[]> {
  const { preguntas } = await postLlm<{ preguntas: string[] }>("/api/suggest", { tables, lang });
  return preguntas;
}
...
export async function fetchExplanation(
  ...
  const { explicacion } = await postLlm<{ explicacion: string }>("/api/explain", { sql, tables, lang });
  return explicacion;
}
